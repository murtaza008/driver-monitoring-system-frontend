...
const FIELDS = [
  ...
  { label: 'Fleet Size', key: 'fleetSize', hint: 'Number of drivers you plan to add, at least 1.' },
...
const AccountManagement = () => {
  ...
  const handleSave = async () => {
    const check = validateCompanyProfile(form);
    if (!check.valid) {
    ...
    <div className="space-y-6 max-w-2xl mx-auto">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-card p-6">
        ...
        <div className="space-y-4">
          {FIELDS.map(({ label, key, readOnly, placeholder, hint }) => (
            <div key={key} className="space-y-1">
              ...
              {editing ? (
                <>
                  ...
                  {!readOnly && <FieldHint>{hint}</FieldHint>}
                </>
              ) : (
